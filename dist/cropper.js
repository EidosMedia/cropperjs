/*!
 * Cropper.js v1.0.0-rc.3 feat Eidosmedia
...
function dispatchEvent(element, type, data) {
  if (element.dispatchEvent) {
    ...
    if (isFunction(Event) && isFunction(CustomEvent)) {
      ...
      if (self.options && self.options.$document) {
        event = self.options.$document[0].createEvent('Event');
      } else {
      ...
      if (self.options && self.options.$document) {
          event = self.options.$document[0].createEvent('CustomEvent');
      } else {
...
function getEvent(event) {
    ...
    if(self.options.$window) {
        e = event || self.options.$window[0].event;
    } else {
  ...
  if (!e.target) {
      if (options.$document) {
        e.target = e.srcElement || options.$document[0];
      } else {
  ...
  if (!isNumber(e.pageX) && isNumber(e.clientX)) {
    var eventDoc;
    if (options.$document) {
        eventDoc = event.target.ownerDocument || options.$document[0];
    } else {
...
function getOffset(element) {
    var doc;
    if (options.$document) {
        doc = options.$document[0].documentElement;
    } else {
  ...
  if (options.$window) {
    return {
        left: box.left + ((options.$window[0].scrollX || doc && doc.scrollLeft || 0) - (doc && doc.clientLeft || 0)),
        top: box.top + ((options.$window[0].scrollY || doc && doc.scrollTop || 0) - (doc && doc.clientTop || 0))
    };
  } else {
    return {
...
function createElement(tagName) {
    ...
    if (self.options.$document) {
        return self.options.$document[0].createElement(tagName);
    } else {
...
function getImageSize(image, callback) {
  ...
  var newImage = createElement.call(self, 'img');
...
function getSourceCanvas(image, data, options) {
  var self = this;
  var canvas = createElement.call(self, 'canvas');
...
var preview$1 = {
  initPreview: function initPreview() {
    ...
    var image = createElement.call(self, 'img');
    ...
    var previews;
    if (self.options.$document) {
        previews = preview.querySelector ? [preview] : self.options.$document[0].querySelectorAll(preview);
    } else {
        previews = preview.querySelector ? [preview] : document.querySelectorAll(preview);
    }

    self.previews = previews;

    each(previews, function (element) {
      var img = createElement.call(self, 'img');
...
var events = {
  bind: function bind() {
    ...
    if (options.$document) {
        addListener(options.$document[0], EVENT_POINTER_MOVE, self.onCropMove = proxy(self.cropMove, self));
        addListener(options.$document[0], EVENT_POINTER_UP, self.onCropEnd = proxy(self.cropEnd, self));
    } else {
    ...
    if (options.responsive) {
        if (options.$window) {
            addListener(options.$window[0], EVENT_RESIZE, self.onResize = proxy(self.resize, self));
        } else {
  ...
  unbind: function unbind() {
    ...
    if (options.$document) {
        removeListener(options.$document[0], EVENT_POINTER_MOVE, self.onCropMove);
        removeListener(options.$document[0], EVENT_POINTER_UP, self.onCropEnd);
    } else {
    ...
    if (options.responsive) {
        if (options.$window){
            removeListener(options.$window[0], EVENT_RESIZE, self.onResize);
        } else {
...
var handlers = {
  ...
  wheel: function wheel(event) {
    var self = this;
    var e = getEvent.call(self, event);
...
var methods = {
  ...
  zoomTo: function zoomTo(ratio, _originalEvent) {
    ...
    /** FIXME: Zoom from mouse position is Blocket due to a different calc mode in the plugin.
    *   if (_originalEvent) {
    *     var pointers = self.pointers;
  ...
  getCroppedCanvas: function getCroppedCanvas(options) {
    ...
    if (options.$window) {
        if (!self.ready || !options.$window[0].HTMLCanvasElement) {
            return null;
        }
    } else {
    ...
    if (!self.cropped) {
      return getSourceCanvas.call(self.image, self.imageData, options);
    }
    ...
    var parameters = function () {
      var source = getSourceCanvas.call(self, self.image, self.imageData, options);
  ...
  createClass(Cropper, [{
    ...
    value: function init() {
      ...
      setData$1(element, NAMESPACE, self);
        ...
        var _HTMLCanvasElement;
        if (self.options && self.options.$window) {
            _HTMLCanvasElement = self.options.$window[0].HTMLCanvasElement
        } else {
      ...
      if (tagName === 'img') {
        ...
        url = element.src;
      } else if (tagName === 'canvas' && _HTMLCanvasElement) {
    ...
    value: function load(url) {
      ...
      if (options.$window) {
        if (!options.checkOrientation || !options.$window[0].ArrayBuffer) {
            self.clone();
            return;
        }
      } else {
    ...
    value: function start(event) {
      ...
      getImageSize.call(self, image, function (naturalWidth, naturalHeight) {
    ...
    value: function build() {
      ...
      var template = createElement.call(self, 'div');
    ...
    value: function noConflict() {
        if(options.$window) {
            options.$window[0].Cropper = AnotherCropper;
        } else {
